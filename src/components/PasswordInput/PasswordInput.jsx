...
const PasswordInput = ({placeholder, name, onChange, value}) =>{
    ...
    return(
        <div className='flex flex-row items-center w-full h-10 pl-2 rounded-lg overflow-hidden mb-2 bg-gray-300'>
            <span className='cursor-pointer' onClick={togglePasswordVisibility}>{
                isPasswordShown ? <Eye className='w-5 h-5 text-gray-500'/> : <EyeOff className='w-5 h-5 text-gray-500'/>
            }</span>
